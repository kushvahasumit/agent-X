...
function PostPreview({
  ...
  const showText =
    checkFilter.text || (!checkFilter.text && !checkFilter.image);
  const showImage =
    checkFilter.image || (!checkFilter.text && !checkFilter.image);
  ...
  return (
    <>
      ...
      {(showText || showImage) && (
        <button
          onClick={() => onPost(editablePost, checkFilter)}
          ...
          disabled={showText && !editablePost.trim()}
        >
          {loading ? (
